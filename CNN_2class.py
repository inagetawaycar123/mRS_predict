...
def load_patient_labels(excel_path):
    ...
    pid_idx = find_col(
        headers,
        ['Patient ID', 'PatientID', 'patient id', 'ID', '编号']
    )
    mrs_idx = find_col(
        headers,
        ['90 Day MRS', '90day mrs', '90 day mrs', 'mRS']
    )
    ...
    patient_labels = {}
    for row in rows:
        pid = normalize_patient_id(row[pid_idx])
        mrs = row[mrs_idx]
        ...
        try:
            mrs_int = int(float(mrs))
        except Exception:
            continue

        if 0 <= mrs_int <= 2:
            label = LOW_RISK_LABEL
        elif 3 <= mrs_int <= 6:
    ...
    print('Excel读取患者数量:', len(patient_labels))
...
def build_slice_samples(patient_ids, patient_slices, patient_labels):
    valid_patient_ids = []
    valid_y = []
    ...
    for pid in patient_ids:
        ...
        valid_y.append(patient_labels[pid])
    ...
    keep_labels = [lab for lab, cnt in zip(unique, counts) if cnt >= 2]
    keep_mask = np.isin(valid_y, keep_labels)

    valid_patient_ids = [pid for pid, keep in zip(valid_patient_ids, keep_mask) if keep]
    valid_y = valid_y[keep_mask]

    if len(valid_patient_ids) == 0:
        print('移除样本过少类别后无可用病人。')
        sys.exit(1)

    print('有效患者:', len(valid_patient_ids))
    print('类别分布:', np.unique(valid_y, return_counts=True))

    stratify_arg = valid_y if (len(np.unique(valid_y)) > 1 and np.min(np.bincount(valid_y)) >= 2) else None
    ...
    for pid, y in zip(train_pids, y_train):
        for slice_path in patient_slices[pid]:
            train_samples.append((slice_path, int(y)))

    for pid, y in zip(test_pids, y_test):
        for slice_path in patient_slices[pid]:
            test_samples.append((slice_path, int(y)))
...
def evaluate_model(model, test_loader, device):
    ...
    print('Classification report:\n', classification_report(y_true, y_pred, zero_division=0, target_names=['low_risk', 'high_risk']))
...
def main():
    ...
    patient_ids, patient_slices = load_patient_slices('np_a_normalized')
    print('病人数:', len(patient_ids))

    patient_labels = load_patient_labels('ProveIt_中文翻译_编码保留版.xlsx')
    ...
    torch.save(model.state_dict(), 'cnn_2class_model.pth')
    joblib.dump({'num_classes': 2, 'class_names': ['low_risk', 'high_risk']}, 'cnn_2class_meta.joblib')
    print('模型已保存: cnn_2class_model.pth, cnn_2class_meta.joblib')
