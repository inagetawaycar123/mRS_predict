...
try:
    ...
    print('请先安装 scikit-learn, xgboost 和 joblib：pip install scikit-learn xgboost joblib')
    raise
...
# 保存模型与 label encoder（如果有）
model_bundle = {'model': clf}
if le is not None:
    model_bundle['label_encoder'] = le
joblib.dump(model_bundle, 'xgb_model_bundle.joblib')
print('模型已保存为 xgb_model_bundle.joblib')